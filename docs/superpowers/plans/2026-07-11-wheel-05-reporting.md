...
### Task 4: Dashboard "Wheel" page

**Files:**
- Create: `dashboard/views/wheel.py`
- Modify: `dashboard/app.py`
- Test: `tests/test_wheel_dashboard.py`

**Interfaces:** consumes `run_wheel`, `wheel_report`, `spy_buy_hold`, `WheelConfig`. Produces `dashboard/views/wheel.py::render()` + a "Wheel" nav entry.

- [ ] **Step 1: Write the failing test**

```python
# tests/test_wheel_dashboard.py
import os
import pytest
from streamlit.testing.v1 import AppTest
from dashboard.views import wheel

FIX = "fixtures/spy_wheel_cycle.parquet"

@pytest.mark.skipif(not os.path.exists(FIX), reason="wheel fixture not built")
def test_wheel_page_runs_and_renders_metrics():
    at = AppTest.from_function(wheel.render).run(timeout=60)
...
- [ ] **Step 3: Write `dashboard/views/wheel.py`**

```python
# dashboard/views/wheel.py
...
import streamlit as st
from dashboard.views import run, wheel
...
Run: `.venv/bin/pytest tests/test_wheel_dashboard.py -v`
Expected: PASS. If `AppTest.from_function` is unavailable in the installed Streamlit, adapt to `from_file("dashboard/app.py")` + `at.switch_page("wheel")`; the INTENT is: the Wheel page renders and a Run click produces ≥1 metric with no exception.

- [ ] **Step 6: Full suite + commit**

Run: `.venv/bin/pytest tests/ -q`
```bash
git add dashboard/views/wheel.py dashboard/app.py tests/test_wheel_dashboard.py
git commit -m "feat: dashboard Wheel page (config -> run -> report + trade log)"
```
...
**Spec coverage:** residual-short settlement fix (Task 1); metrics/recent/yearly/benchmark/stats (Task 2); printable formatter (Task 3); dashboard Wheel page (Task 4). Standing methodology (recent headline + year-by-year) honored in `wheel_report`/`format_report`/the page.

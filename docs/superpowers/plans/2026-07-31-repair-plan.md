...
**Created:** 2026-07-31 · **Status:** **DEPLOYED, RESET, AWAITING DAY 1.** (Session 5, 2026-08-03: code backed up to private GitHub `GeorgeManavazian/wheelbot-code` — the repo had NO remote · F4a DONE `da803b6` · F7 reset tooling DONE `faedf9f` and EXECUTED on the VPS · F5 deploy DONE, 41/41 byte-identical, VPS suite green · two live defects caught by deploying: git-identity `903ade0`, PAT workflow-scope seeded.) **BLOCKING DAY 1 — owner actions only:** (1) `ssh ubuntu@129.80.185.142` → re-run the Schwab login (VPS token is 7.99d = EXPIRED; the 08-02 login was on the Mac) (2) enable the timer. **Then remaining:** F8 timer semantics + one `workflow_dispatch` run (after day 1 succeeds) · A20 · A10d · Q1-Q7 · 42 unverified F4 findings retained as a LOOKUP TABLE for when the live bot misbehaves, not a work queue (owner ruling: stop auditing, run live and watch). Suites **586 + 405** (Mac), **384+1skip** (VPS). · **Bot state:** PAUSED, fresh store, epoch 2026-08-03
...
| A21 | Held-leg quote pull happened at 17:00, post-close — same staleness class as A16 but feeds the EOD TP branch, so it was a TRADING input | MED | **DONE** | see the A21 evidence block below. Owner rulings 2026-08-02 (D1 RTH for marks+TP / D2 carried-marks day / D3 anchor break blessed) implemented per the analyst's option (b). Skeptic CORRECT-BUT-INCOMPLETE → 6 amendments same session (F1 MAJOR runner-crash guard, F2 carry-forward, F3 malformed-stats, F5 executed store receipt, F6 rows-over-stats, F7 drift disclosure); F4 declared. 15/15 mutants killed incl. one genuine lint-blind survivor. Suites 586+376. |
...
| A9b | Router trend→chop share-handoff can sell a covered call in the handoff session (`regime_router.py:165-170` sets phase="CALL", `assigned_today` stays False). Unlike assignment, those shares settled long ago — same-day sale is physically fillable. | LOW | **DONE (owner ruling 2026-08-02: KEEP AS-IS, declared)** — the sale is physically fillable (shares settled long ago), live never routes trend, and the sim is not distorted for cosmetic uniformity. A9's deferral rationale (shares arrive overnight) does not apply here. No code change; this row IS the declaration. | filed from the A9 skeptic, 2026-08-02. Solo-only surface (batch router; live never routes trend). `liquidate_assignment` same-day LIQUIDATE+re-entry also noted — research flag, rejected by both production engines, out of scope. |
| A10 | Corporate actions — at minimum detect and refuse | HIGH | **DONE (owner CONFIRMED 2026-08-02)** | commit `b672cda`; analyst spec + A10 evidence block below. Restatement detector (`LiveMarket.prior_close` vs stored `last_spot`, capability-gated so batch is byte-identical by construction) → sticky manual-clear freeze; ≥25% gap with clean restatement → ONE deferred settlement session + 5-session watch (real crash auto-clears, tested); intraday underlying-gap refusal + frozen-skip; state round-trip; one deduped daily FROZEN email. **D1-D4 (band 0.80/1.25, backstop 25%, manual clearing, no entry veto) CONFIRMED by owner in the ELI10 dialog — freeze-and-ask over self-correct, understood and blessed.** Mutants X1-X6 killed. Suites 560+295=855. |
...
| B6 | Re-vet the universe; add a `_RETIRED` frozenset with a test | HIGH | **DONE** | commit `e4d68ac`. Evidence sweep (read-only agent) → kill-list → **owner approved ALL 11** (2026-08-02): PARA (dead since 2025-08-07, vendor serves a frozen $11.04 close that RANKED normally — the silent-staleness class itself), WBA (private), GPRO/PLUG (sub-$1), CHPT (1:20 reverse death spiral), + surgery-scarred SIRI/AMCR/DD/DOC/FLEX/FTV. `_RETIRED` frozenset, subtract-don't-delete, leak test; universe 542→531; 2 mutants killed; none of the 11 held (checked vs the real synced book, 9 legs). **Declared (sweep):** 210 universe names have zero local options data and are locally unvettable — a live re-check belongs on the resume-day checklist. |
...
| ID | Fix | Sev | Status | Evidence |
|---|---|---|---|---|
| C1 | Record per-leg quote timestamps; flag/refuse bars with a stale book | CRIT | **DONE (capture+flag; refusal thresholds deferred by design)** | Analyst spec + measurement: on real captured chains the STALEST quotes are the deep-OTM 0.00×0.01 winners (quoteTime updates on NBBO change) — age-refusal would re-blind exactly what A6 unblinded, so day 1 = capture everywhere + carried-mark discrimination, zero new refusals (A19 doctrine; OD1-OD4 defaults taken). Landed: `quote_time`/`trade_time` chain columns (chain payload keys proven on the GDX capture) + held-leg dual-key capture; `Mark.quote_time` threaded (backtest frames → None → byte-identical, 586 green); portfolio §3 stamps `mark_asof`+`mark_quote_time` on every RE-mark, never on the carried path and never on frozen legs (A10f) — `mark_asof < today` IS the TMO-$11.30 discriminator; state/snapshot round-trip; dashboard "Mark as-of" column (live / — / red `carried <date>`); run_daily prints snapshot `pulled_at` age (field was write-only since day one). **C1b found+fixed (HIGH):** the A7 intraday gate read `quoteTimeInLong` from the QUOTES endpoint whose spec name is `quoteTime` — a spec-shaped payload would refuse EVERY leg on resume (all intraday TPs suspended, fail-closed); dual-key now. **VERIFY RESOLVED 2026-08-02 (Phase F step 0): live capture of 3 real held OCC symbols + SPY — the quotes endpoint serves `quoteTime`/`tradeTime` (epoch ms), `quoteTimeInLong` absent; the A7 gate's primary key is CORRECT. Capture: scratchpad `c1b_quotes_capture.json`. Expired TMO 260731 returns via the `errors` key (consistent with unquoted handling).** Declared: pre-C1 snapshots fail load validation (A19-F4 class, zero risk while paused); nothing here prejudges A21 D1/D2/D3 (capture is measurement, not a ruling — the same-day RTH-vs-17:00 staleness pair A21 wants now accrues automatically); minutes-amber threshold = future row on ≥2wk of accrued data. Gates: red-first ×5 surfaces, suites 586+341, 6 mutants killed (dual-key severed ×2 sites, capture dropped, always-stamp, round-trip dropped, Mark-thread dropped). |
| C2 | Two disclosure classes; render the reason, not just the date | CRIT | **DONE** | with C3, one change: `split_gap_records()` — plain `no_run` = real hole ("nothing ran, cannot backfill"); corrections/any other reason = "the bot DID act, this is the caveat its numbers carry" with the reason rendered per date. Kills the banner text that was false for 45.5% of realized P&L. Mutant (corrections lumped into full-miss) killed. |
| C3 | Banner must render `records`, not `dates` | HIGH | **DONE** | banner consumes `g["records"]` through the split; source pin forbids `g["dates"]` in the banner body. Live 346. |
| C4 | Suppress Sharpe below ~30 obs; show ±SE; subtract rf; fix annualisation | HIGH | **DONE** | Group C final batch block below (session 4). `_sharpe_stats`: suppressed < 30 obs (dash rendered WITH n), ± one SE (Lo 2002), rf 4.0%/yr subtracted, annualised by ACTUAL observation frequency of the deduped index (HEAD rendered Sharpe 99.48 from 3 obs, executed receipt). |
| C5 | Campaign-level win rate on realized cash; show wins/losses/open | HIGH | **DONE** | same block. Win = realized cash > $0 per (ticker, campaign) from cash_after deltas seeded at CAPITAL; open campaigns excluded and shown apart. Old action-name rule was structurally 100% (a losing CLOSE_PUT counted as a win — re-pinned red). Skeptic P1: independent recompute, all 25 real accounts, every campaign, 0 failures. |
| C6 | Buy-and-hold benchmark (SPY + equal-weight of names traded) | HIGH | **DONE** | same block. Nightly `benchmark.jsonl` row (SPY + every ledger name, closes from the SAME pull the decisions used) via extracted `write_benchmark_row` (unit-tested + wiring lint pin, C12-X3 lesson); dashboard line ALWAYS carries the capital-matching caveat. Coverage begins at first write — no backfill (docstring corrected). F3 declared: EW basket = names present at BOTH window ends; copy says so. |
| C7 | Header the grid with effective-N, distinct decisions, top-underlying share | HIGH | **DONE** | same block. `grid_concentration`: N_eff = k/(1+(k−1)·ρ̄) clamped at ρ̄≥0 (clamp pin added — anti-correlation must not inflate N_eff past k), distinct-decision count + replication + top-underlying premium share; young store SAYS it cannot estimate, never goes blank. Real store: N_eff 2.086, ρ̄ 0.458 — reproduces the audit. |
| C8 | One source of truth for both dashboard pages, or stamp as-of dates | MED | **DONE** | same block. As-of stamps taken (dual-source kept, declared): board caption = state.json mtime + last snapshot date; compare header = min/max snapshot as-of across accounts; snapshot rows now carry `written_at`. A board-vs-compare difference is two clocks, not a contradiction. |
...
| C10 | Drawdown/returns measured from capital, not the first snapshot | MED | **DONE** | Group C final batch block below. Synthetic day-0 row at starting capital, strictly BEFORE the first real date (same-date prepend mutant: C12's dedupe eats the real row). HEAD receipt: first snapshot 99,671.90 on 100k rendered Max DD 0.00%. Post-fix: nonzero DD for 25/25 real accounts. |
| C11 | Stamp `mark_basis` in every snapshot; annotate the changeover | MED | **DONE** | same block. Engine constant `MARK_BASIS` (portfolio.py) stamped into every snapshot row — single source, hardcode mutant killed; dashboard `mark_basis_line` re-pinned after skeptic F2: unstamped rows = UNKNOWN basis (never a guessed "mid" — the mid→ask changeover predates stamping), a change between two STAMPED rows is named at the exact row it happened. |
| C12 | Deduplicate the equity index | LOW | **DONE** | last-row-per-date at BOTH consumers: `_equity_series` (compare/metrics — returns, drawdown, Sharpe ppy all double-counted the duplicated 2026-07-24 rows) and `dedupe_snaps` on the account board. 3 mutants killed incl. a survivor (board call-site pin added after X3 walked through the unit tests — the E8 lesson again: wiring needs its own pin). |
...
| C16 | Surface `days_shares_uncovered` per account on the dashboard (incremented + persisted, zero live readers) | MED | **DONE** | `naked_days_line()` pure helper + `st.warning` on the account board, only when nonzero; unit test + wiring pin; always-None and wiring-drop mutants killed; live 328. |
| A21b | Chain store drops unknown columns on load — flags die on round-trip (from the A21 analyst, 2026-08-01; fix inside A21's implementation) | MED | **DONE (inside A21)** | held rows never enter the chains dict (separate `held_rows` key, plain dicts, no DataFrame rebuild) AND `held_only=True` is FORCED at load — the flag survives because it is re-imposed, not round-tripped. Tampered-flag test + trust-file-flag mutant killed. The generic chains-drop behavior remains but is inert (only `save_chain_snapshot` writes the file and it writes exactly `_CHAIN_COLS`) — declared. |
| A10b | Dead OCC symbol (reverse split/symbol change/delisting) sits `unquoted` indefinitely, TP suspended, print-only — needs N-consecutive-days escalation (C16b class) | MED | **DONE** | with C16b, one mechanism: `live/escalations.py` (see C16b row) |
...
| A10e | Batch chains may contain unadjusted CA fossils beyond the one XOP `DEFAULT_CLEAN_START` fence (~19 candidates found in underlying histories) — backtest integrity sweep | LOW | **DONE** | Sweep agent: 341 chains scanned, 86 band hits → **26 FOSSILS** (splits, reverse splits, spinoffs, 2 symbol-reuses) vs ~45 real crashes (ladder-overlap discriminator) + 13 multi-week pull-gap drifts (separate data-quality note, not CA). **Every used decision number verified clean** (SEEN-4 router verdicts, all 8 chop-gate A/B arms re-run, FROZEN provenance, A20 evidence, pending basket unblocked). **CONTAMINATED: the solo equal-weight baseline** in the rotation report — NVDA −$363k phantom, META ×15 wrong-instrument jump (pre-2022-06-09 "META" is the Roundhill Metaverse ETF) — any citation of it is invalid until re-run. Fix landed: `DEFAULT_CLEAN_START` += AAPL/AMZN/META/NVDA (fossil+~2mo, XOP precedent; flows to solo arms via the rotation script's trim), fence-vs-last-fossil pin test, dropped-entry mutant killed, suites 582+336. Declared: NVDA fence loses 2010-2024 history — re-pull adjusted to restore; expansion-341 fence map lives in the sweep's `verdicts.csv` (regenerate before any expansion run); solo baseline re-run deferred to owner (invalidated number, not yet replaced). |
...
| C17 | `last_spot=0.0` positions render −100% "ITM" on the dashboard (HAL/WBD, real) | LOW | **DONE** | no spot (0.0/absent) → `_dist=None` → renders "—" (the bare-shares sentinel the formatter already handled); red-first, revert-mutant killed. |
| A10f | A `ca_frozen` position's short leg is still re-marked every step — portfolio.py section 3 (~:461-482) has no frozen check, so `last_ask`/`last_mid` are overwritten daily from a possibly-restated chain and snapshot/dashboard equity swings in unknown units during a freeze (state.json contaminated mid-restatement; batch residual finalizer would buy back at it). No trading decision reads it (frozen skips TP/settlement/call). | LOW-MED | **DONE** | frozen legs carry the pre-freeze mark into liab (never re-marked) until manual clear; red test `test_frozen_position_keeps_its_prefreeze_mark` (pre-fix: 2.10→4.40 overwrite reproduced) → guard-removal mutant killed → suites 581+319. Declared: frozen-book equity is the CARRIED (stale, pre-freeze-unit) value, not live — consistent with "every stored number on a frozen position is in unknown units". |
| C16b | Escalation for the A3b refusal class: N consecutive `call_gated_unclosable` days on one ticker → email (today it logs daily, forever, and never emails — while A4's floor-above-window class emails daily; same physical condition, two tiers). Needs persisted per-ticker consecutive-day state; owner picks N. | MED | **DONE** (provisional N=3) | commit pending this row. `live/escalations.py`: persisted consecutive-STEPPED-day counters in the synced store; a kind passed with an empty list = surface ran clean = streak reset; same-day re-calls idempotent (a partial-failure retry can't zero a streak); stepped-day gaps don't reset (a VPS outage doesn't launder a dead symbol, declared); keys global across the 25 accounts (market/data condition, not sizing — alternate-universes ruling is about liquidity, declared). Wired in run_daily for BOTH kinds (A10b `unquoted_leg` from `merged["unquoted"]`, C16b `call_gated_unclosable` from per-account warnings), smoke-gated (A23 class), one alert per stepped day at ≥N repeating while present (FROZEN-alert shape). Red-first wiring pin + 5 unit tests; 5 mutants killed (same-day reset, double-count, off-by-one threshold, reset-removal, smoke-gate drop); suites 581+325. **N=3 CONFIRMED by owner 2026-08-02 (ELI10 dialog).** |
| E8 | `run_chain_snapshot.main()` wiring tests (zombie wiring, partial-exit-1, save-recheck call site, `--force`) — predicates are tested, the wiring is executed only by the A16 skeptic's S1 run and the C2 dry run | **DONE** | 6 wiring tests via a monkeypatch harness (fake market/client/store, sequenced clock for the slow-pull case, no network, no real store): zombie→exit 1 nothing saved · partial→SAVED + exit 1 (F3 semantics) · slow pull past 16:05 grace→discarded · clean→saved exit 0 · `--force` outside window pulls+saves · no-force outside window refuses before client. Non-vacuity: 4 wiring mutants (zombie branch dead, partial exits 0, recheck dead, force ignored) each fail their test. Live 334. |

---

## A21 — evidence block (2026-08-02, session 4; owner D1/D2/D3 ruled same session)

**In plain language.** The bot's evening self-check ("should I buy back this option I sold?")
was still reading one input off the dead 5pm board: the prices of options it already holds.
Now the 3:20-3:50pm photo pass also photographs every held option (asking Schwab by exact
contract ID), stores those quotes beside the chain photo, and the 5pm run reads the photo —
it never calls the quote endpoint again outside `--smoke`. If one day the held-photo fails all
window, the day still runs on yesterday's prices with the buy-back check paused and one email
sent (owner D2). History priced before/after this change is not comparable (owner D3, blessed;
Phase F discards the old rows anyway).

**What landed:** `held_legs.py` split into `pull_held_quotes` (pull half, never raises — incl.
the contract derivation, skeptic F1) + `merge_pulled` (merge half) with `merge_held_legs` kept
as their composition (smoke + seam reference) · `chain_store.py` gains `held_rows`/`held_stats`
top-level keys (NEVER inside chains — the chains round-trip drops unknown columns, A21b) and
`load_held_rows` which FORCES `held_only=True` at load (the flag is a security boundary; it is
re-imposed, not trusted) and returns None on any malformed byte (F3) · `run_chain_snapshot`
pulls held quotes in-window before the save-clock recheck, saves them with the snapshot,
treats raise/answered-nothing as a failed pull (chains still saved, exit 1, in-window retries)
and carries an earlier tick's good held rows forward through a later failed tick (F2) ·
`run_daily` routes through `resolve_held_merge` (4 branches: smoke live-probe / no-snapshot
inert / D2 carried-marks day with ONE alert / merge-from-store), D2 days bypass
`held_marks_failed` (immutable snapshot — the A16-F2 retry-spam class), omit `unquoted_leg`
from escalations (a failed surface must not launder a dead symbol's streak), and disclose
position-set drift between 15:2x and 17:00 (F7).

| Gate | Evidence |
|---|---|
| 1 Reproduce | 6 red-first tests (store round-trip, absent-vs-empty, seam equality, wiring, D2, runner), all defect-shaped on the pre-fix tree; behavioral probes: HEAD's 17:00 path calls get_quotes (the RIG $0.01-vs-$0.06 receipt at filing). |
| 2 Minimal fix | The five pieces above; A21b folded in by design (held rows never enter the chains dict). No opportunistic cleanup. |
| 3 Suites | **586 backtest + 376 live** green (376 = 371 + 5 amendment tests). |
| 4 Mutation | **15/15 killed**: MH1 trust-file-flag, MH2 force-D2 (a GENUINE survivor — lint could not see dead code; killed by the behavioral branch test after extracting `resolve_held_merge`), MH3 D2-alert-drop, MH4/MH4b runner error-branch, MH5 unquoted-drop, MH6 escalation-omit-drop, MH7 held_d2-guard-drop, MH8 merge-branch-dead, B6-style lints, + amendment mutants AF1 unguarded-contracts, AF2 no-carry-forward, AF3 TypeError-uncaught, AF6 stats-gate-revert, AF7 drift-dead. |
| 5 Line audit | Every hunk authored this session and accounted to D1/D2/D3/A21b/F1-F7. |
| 6 Blast radius | `save_chain_snapshot`: runner + tests (kwargs optional, old callers unchanged). `load_held_rows`: resolve_held_merge + runner F2 branch. `merge_held_legs`: smoke branch + tests only. `held_marks_failed`: unchanged predicate, now guarded by held_d2. Escalations: omitted-kind semantics confirmed in module contract. |
| C1 Skeptic | **CORRECT-BUT-INCOMPLETE** → 6 amendments same session (F1 MAJOR: one malformed position crashed the entire snapshot runner — guard moved inside `pull_held_quotes`; F2: later-tick clobber of good held rows — carry-forward; F3: corrupt held_stats raised at 17:00 — coercions inside the guard; F5: success path pinned only by regexes — executed store receipt added; F6: empty stats silently dropped real rows — gate on error only; F7: silent position-set drift — disclosed). Could-not-break: seam equality 0 divergences on hostile payloads; store round-trip byte-identical through `select_contract`/`option_mark`; real `step_one_day` D2/D1 semantics exact (in-window leg TPs at RTH ask 0.01 — the RIG shape); escalation streak survives a D2 omit and fires on the 3rd stepped observation; 26 real states round-trip clean. |
| C2 Dry run | Live pull impossible (bot paused, no snapshot file exists anywhere — the runner reaches the VPS only at Phase F; local token expired — DECLARED). Substitute real-data receipts: pull-half derives all **9 distinct held legs from the real 26-account synced book** (TMO 512.5P and RIG 4.5P among them); skeptic ran the full pull→save→load→merge round-trip on the real book, clean. |
| C3 Dollars | **$0.00 today** (paused, nothing steps). Prospective: the EOD backstop TP now prices off the RTH book — the class of difference measured at A21 filing: RIG 4.5P RTH ask $0.01 fills, post-close $0.06 refused. |

**Declared (F4):** the D2 alert is per-invocation, not per-day — on an evening where a
co-occurring exit-1 condition keeps the tick retrying, it repeats. Co-failure evenings are
already loud; dedupe-by-day is a cheap follow-up if Phase F rehearsal shows it grates.
**Phase-F note:** first deployed snapshot day writes the first held_rows file; until then every
resumed day is a (correct, alerted) D2 day — expect exactly one HELD-LEG MISSING alert on any
day the snapshot pass predates the deploy.

---

## GROUP C FINAL BATCH — C4+C5+C6+C7+C8+C10+C11 evidence block (2026-08-02, session 4)

**Provenance:** the tree was found dirty AGAIN (4th time) with this whole batch in flight and no
plan entry. Standing rule applied — nothing trusted: every new test re-proven red on HEAD in a
worktree (12/12 red, defect-shaped receipts: HEAD renders Sharpe **99.48 from 3 observations**;
a first-snapshot dip below capital shows **Max DD 0.00%**; the old win-rate rule scores a losing
buy-back as a win), coverage holes plugged, then gated as this session's own work.

| Gate | Evidence |
|---|---|
| 1 Reproduce | 12 new/re-pinned tests, all red on HEAD (worktree at `bb7e680`), key reds behavioral (above); C6/C7/C11 feature-absence reds backed by executed HEAD probes (no benchmark store, no N_eff, snapshot rows carry no basis stamp). |
| 2 Minimal fix | Inherited batch + session-4 additions: `write_benchmark_row` extraction (inline-in-main was untestable), C7 clamp pin, benchmark-store docstring corrected (claimed retroactive completeness it does not have), 3 skeptic amendments (below). No opportunistic cleanup. |
| 3 Suites | **586 backtest + 362 live**, both re-run full after every amendment round. |
| 4 Mutation | **16/16 killed**: rf-drop, flat-252, suppression-off, prev-seed-zero, open-not-excluded, same-date prepend, no-prepend, hardcoded basis, clamp-drop, smoke-gate-drop, caveat-drop, wiring-drop, + 4 amendment mutants (fmt `is None` ×2, unknown-branch drop, change-misfire). Backup-restore harness (never `git restore` — it would have wiped the uncommitted batch), `__pycache__` nuked between runs (stale-pyc rule). |
| 5 Line audit | Full diff read line-by-line pre-verification; every hunk accounted to a row ID; `_paths`/`in_state`/`_closes` wiring verified against existing call-site idioms (run_daily:107 precedent). |
| 6 Blast radius | `MARK_BASIS`: engine + snapshots.py + tests only. `_equity_series`: 2 internal callers (grid_concentration deliberately WITHOUT capital — a shared synthetic day-0 would inject fake cross-account correlation). `account_metrics`/`grid_concentration`/`benchmark_series`: dashboard render only. Nothing on the money path. |
| 7 Plain language | Delivered in chat, session 4. |

**Skeptic (fresh, read-only): BROKEN (narrowly) → 3 amendments same session, each red-first + mutant-killed:**
- **F1 HIGH** — `pd.DataFrame` coerces `None`→`NaN` in float64 columns, so the grid's `is None`
  guards were dead code: the REAL store rendered **`nan% (0W/0L)`** (executed receipt, 5k_N5).
  Unit tests missed it because they never crossed the DataFrame boundary. Fixed: NaN-safe
  `fmt_sharpe`/`fmt_win` formatters judged with `pd.isna`, boundary-crossing test; also covers
  the F6 leg (≥30-obs curve whose equity touched 0 → NaN Sharpe must render suppressed, never
  "nan ± nan"). Post-fix real-store receipt: `— (n=4) | —`.
- **F2 MED** — `mark_basis_line` claimed "changed mid-series" on the real all-unstamped store
  (nothing changed) and mislabeled the changeover boundary. Rewritten: unstamped = UNKNOWN basis
  (never a guessed "mid"), stamped-to-stamped change named at the exact row; 3 boundary cases
  pinned. Post-fix real-store receipt: honest unknown-basis line.
- **F3 MED** — EW benchmark basket = names present at BOTH window ends; a name first traded
  mid-window is silently absent. Render copy now says "held at both window ends"; declared.

**Skeptic could-not-break (receipts):** independent per-campaign P&L recompute + telescoping
identity (Σ campaign pnl == last cash_after − capital) across **all 25 real accounts, every
campaign: 0 failures**; full assignment cycle (SELL_PUT→ASSIGNED→SELL_CALL→CALLED_AWAY)
classified right incl. interleaved campaigns; C10 nonzero DD 25/25; C7 reproduces the audit's
N_eff 2.086/ρ̄ 0.458 in 0.03s; snapshot round-trip clean through dedupe/equity/already_stepped;
no strict-schema snapshot consumer; tests-not-weakened confirmed (only the two declared win-rate
re-pins changed existing assertions).

**Owner review DONE — batch BLESSED 2026-08-02 (session-4 ELI10 dialog). Group C CLOSED**
(F3-dashboard-actually-opened remains a Phase-F checklist item by design).

**Declared, not fixed:** (F4) a missing-`cash_after` row folds its cash move into the NEXT
judged row's campaign — unreachable post-reset (`_trade_row` always stamps it; verified on all
25 real ledgers), fixture-only; (F5) exactly-$0 net campaign counts as a LOSS (strict `> 0`,
fail-safe direction); (F6) zero-vol curve renders "0.00 ± big" (rare, cosmetic); (F7) a retry
tick after partial failure appends a second same-day benchmark row — read-side last-wins dedupe
absorbs it, file growth only; (F8/F9) None-date benchmark row / str-typed campaign key misbehave
but no current writer can produce either; C8 keeps two sources WITH as-of stamps rather than
unifying (timing difference now self-explains); benchmark store coverage begins at first write —
day 1 post-reset — no backfill exists.
...
**Post-sweep suites: 580 backtest + 319 live green.** **Owner review DONE — BLESSED 2026-08-02 (session-4 ELI10 dialog). Group A boundary CLOSED.**
...
the deliverable is defense IF it ever happens under real money. Defaults taken provisionally,
then **CONFIRMED by owner in the session-4 ELI10 dialog (2026-08-02): D1 freeze+manual-clear
and the D2 $25 dust-vs-real line both understood and blessed** · D3 flag in
`data/live/config.json` · D4 reconcile every run both flows (at real-money build) · D5 land
the seam now.
...
| # | Gate | Status |
|---|---|---|
| F1 | Full-day rehearsal, all accounts, throwaway store, live data, nothing written to real state | **NOT RUN — owner ruled 2026-08-03: treat day 1 itself as the rehearsal, watched.** Partly substituted: the full live suite was executed ON THE VPS post-deploy (384 passed, 1 skipped) and F2's forced-assignment drill already exercised `step_one_day` through the live persistence layer on real data. Declared gap: no all-25-account run against live Schwab chains has ever executed on the VPS. |
| F2 | Forced assignment test on a scratch account: expiry → assignment → basis floor → covered call → called away, on real data, ledger shown to owner | **DONE 2026-08-02** — real GDX history (2,392 sessions) through the SHARED `step_one_day` with the LIVE persistence layer round-tripped every day on a scratch store: 33 campaigns, **7 full-lifecycle**; campaign-4 ledger walked with the owner (SELL_PUT → ASSIGNED $101,200 out → **A9 one-session deferral observed live (Fri assign, Mon call)** → 8 covered-call cycles, basis floor never violated → CALLED_AWAY, +$2,771); every cash_after hand-verified to the cent incl. $0.65+$0.05 friction. Declared: A2 liquidity gate disabled for the drill (parquet has no OI/volume; its fail-closed refusal on blind data was itself observed first — correct). |
| F3 | Dashboard actually opened and read (never done as of 2026-07-31) | **DONE 2026-08-03** — `scripts/dashboard.sh` (mirror pulled: last VPS push "intraday 2026-07-31", consistent with the pause) + both pages opened in a real browser and read. Confirmed rendering on the real store: gap banner two-class split (C2/C3), as-of stamps board vs compare (C8), mark-basis unknown-rows note (C11), "Mark as-of" column with `stale` flag on HAL 31.5P (C1), Sharpe suppressed "— (n=7)" (C4), win rate with W/L + open-separate (C5), concentration header N_eff≈2.1 / ρ̄ 0.46 / DOW 49% (C7), nonzero Max DD all rows (C10). C6 benchmark section absent — correct per "coverage begins at first write" (benchmark.jsonl does not exist yet; `benchmark_line` returns None). **Finding F3a:** the audit-2 #3 gap corrections NEVER reached the authoritative ledger — synced `gaps.jsonl` still holds the 3 original records (07-24 = plain `pull_failure`, 07-27 absent entirely); `stale_marks_recorded_as_traded` exists only in tests; Mac frozen store has no gaps.jsonl at all. The banner truthfully renders a ledger that is itself uncorrected, and _STATUS's "#3 FIXED" claim is code-only. Decide at F7 whether the reset wipes gap history (moot) or the corrections must be applied to the VPS ledger on deploy. |
| F4 | Fresh nine-domain audit of the repaired system, from scratch | **PARTIAL 2026-08-03 — 5 of 9 domains found, 0 verifier passes ran. Findings: `docs/superpowers/AUDIT-2026-08-03-F4-raw-findings.md`.** The 2026-08-02 run was lost with its session (zero findings written). The 2026-08-03 re-run hit the model usage limit mid-flight, then was **deliberately stopped for cost** — owner's call: run the audit in SECTIONS, one domain at a time, not as one large fan-out. Landed: **43 raw findings** across security (×2 independent passes), reporting, state-persistence, data-quality, infra-ops. **Owner ruling 2026-08-03: run the money-path domain alone, then stop auditing.** Money-path launched as a single sectioned finder (its findings verified inline by the main session, no verifier fan-out). **alerting, test-integrity and declared-honesty are DROPPED by owner decision on cost** — that is an accepted, declared blind spot, not a clean bill. Every finding is UNVERIFIED except those in the audit file's verification log. **Do not treat this row as done, and do not read a small finding count as a clean bill.** |
| F4a | **DEPLOY BLOCKER — a staged file DELETION wedged the mirror sync permanently.** `secret_guard()` resolved staged paths against the worktree, so a deleted-but-staged file raised `FileNotFoundError` and D8's "unreadable == offender" rule reported it as a credential leak: sync aborted, reset, returned False, and since the file stays deleted the identical abort repeated every tick forever. D11's 7-day `.bak-*` prune deletes a *tracked* file (sync.py deliberately keeps `*.bak-*` out of `.gitignore`), so this armed on the first post-deploy run and fired ~8 days later. | CRIT | **DONE 2026-08-03 (`da803b6`)** | F4a evidence block below |
| F5 | Deploy; verify Mac/VPS byte-identical by checksum | **DONE 2026-08-03** — rsync of `live/ src/ deploy/` (--delete) + `scripts/wheelbot_tick.sh` + `scripts/schwab/`; zero deletions proposed (purely additive). **41/41 files byte-identical by sha256** (`live/*.py`, `src/engine_v2/options/*.py`, the tick script). Collected-test sets compared: Mac 402 vs VPS 384+1skip, difference is EXACTLY the 18 `test_dashboard_calc.py` tests, skipped by declared design (streamlit/plotly are Mac-only on a 954MB box). systemd units installed to `/etc/systemd/system` + `daemon-reload`, all three sha256-verified, timer left `disabled`/`inactive`. **Found by this check:** the live suite passed on the Mac and FAILED on the VPS — git identity was set only on sync's init branch, so a repo created any other way had none and `git commit` dies with "Author identity unknown" on a box with no global gitconfig. Production was not wedged (the VPS store carries a local identity from its manual creation); fixed anyway at `903ade0`. |
| F6 | Re-enable timer; run the service once to re-anchor; confirm `NEXT` exists | TODO |
| F7 | Reset all 25 accounts; day 1 begins | **TOOLING DONE + EXECUTED 2026-08-03** (`live/reset.py`, `faedf9f`). Owner ruling: "act like it's our first time running the bot on live data" → archive, do not delete. Executed on the VPS: 4 entries moved to `data/archive-2026-08-03/` (26 account dirs, `gaps.jsonl`, `logs/`, the 07-18 testrun), `config.json` PRESERVED in place and copied to the archive (it is operational config — n/capital/zombie_threshold/real_money — not track record), `.git` kept so the mirror keeps its history. Fresh store = empty `accounts/`, `logs/.epoch-2026-08-03`, config.json. **F3a resolved by this: the uncorrected gap ledger was archived, so there is nothing to correct.** The reset staged 142 deletions and the production `sync_state` committed and pushed them cleanly — F4a proving itself on the real store, since before that fix this reset would have wedged the mirror permanently on its first tick. Day 1 has NOT begun: the timer is still off. ~~no reset tooling exists.~~ Superseded detail: Checked 2026-08-03: there is no reset script and no account-initialisation function anywhere (`live/accounts.py` only *derives* paths from `CAPITALS × NS`; nothing creates or clears a store). The reset is currently an undefined manual `rm`. Decide and write it down before day 1: which paths get cleared (state.json, trades.jsonl, snapshots.jsonl, the `.bak-*` set, benchmark.jsonl), and **whether `gaps.jsonl` is wiped** — that is the decision that moots or keeps F3a. Note `accounts/_smoke` is a 26th dir inside the same root and is force-pushed to the mirror; the reset must say what happens to it. |
| F8 | **Group D deploy checks** | **PARTIAL 2026-08-03.** DONE: units installed + daemon-reloaded + sha256-verified; the **PAT `workflow`-scope trap CONFIRMED IN PRODUCTION and resolved without a new token** — the first real `sync_state` was rejected with `refusing to allow a Personal Access Token to create or update workflow .github/workflows/mirror-freshness.yml without workflow scope`, which would have bricked the mirror on day 1's first tick. Fixed by seeding the workflow into the mirror once with a workflow-scoped credential from the Mac; D14's self-install only writes when content DIFFERS, so the bot's PAT never re-stages it. Re-ran: `sync_state -> True`. **Standing constraint (declared): any future EDIT to `deploy/mirror-freshness.yml` will re-arm this — it must be seeded the same way, or the PAT must gain `workflow` scope.** Workflow confirmed registered and `active` on the mirror. NOT DONE: timer semantics (needs the timer on) and the one `workflow_dispatch` test run — deliberately deferred, since on a fresh store pre-day-1 it would correctly report STALE and fire a false issue + email. Run it after day 1's first successful run, when a PASS is the expected result. |
| F9 | **Resume-day alert flood:** the forward scan will record EVERY paused weekday since 07-31 as a permanent `no_run` gap and email them all; old gaps 07-21/07-23 also re-email if unexpired. | **DECIDED (owner 2026-08-02): ACCEPT THE FLOOD.** "Let it bark" — no pre-seeded markers, no paused correction. Resume night will deliver ~20 honest alarms in one burst; noted that this was chosen over the truthful-correction option with the alarm-fatigue risk explained. Phase F checklist: expect the burst, do not treat it as a failure. |

---

## F4a — evidence block (2026-08-03, session 5; commit `da803b6`)

**Root cause, one sentence:** the guard asked the *filesystem* what was being pushed, when the
thing being pushed is the *index*.

Three failure modes, one defect:
1. **Deletion.** `os.path.join(state_dir, p)` on a staged deletion → `FileNotFoundError` → D8
   offender → abort → `git reset` → the file is still deleted → identical abort next tick,
   forever. D11's `BACKUP_KEEP_DAYS = 7` prune of a *tracked* `*.bak-*` guarantees the trigger.
2. **`core.quotePath`.** `--name-only` escapes a non-ASCII path to `"logs/na\303\257ve run.log"`,
   which resolves to nothing on disk — same fail-closed wedge, triggered by an accented filename.
   Fixed by `-z` (git never quotes under `-z`).
3. **Worktree vs index** (`security:guard-scans-worktree-not-index`, MED, same file, same line).
   `git add -A` snapshots into the index; the commit ships the index; the guard read the
   worktree. A body holding a credential at add-time and overwritten before the guard ran was
   pushed with the secret intact and reported clean. Fixed together — reading the index is what
   makes the deletion case *definitionally* exempt (no index entry = no bytes pushed) rather
   than special-cased.

| Gate | Evidence |
|---|---|
| 1 Reproduce | 10 new tests in `live/tests/test_sync_deletion_wedge.py`, **6 red on HEAD for the defect** (not import/typo): `[secret_guard: accounts/100k_N1/state.json.bak-2026-07-20 unreadable (FileNotFoundError) -- treated as an offender]` → `[sync ABORTED -- credential in staged paths: [...]]`, and the quotePath twin `[secret_guard: "logs/na\303\257ve run.log" unreadable ...]`. |
| 2 Minimal fix | `live/sync.py` only. `_staged_entries()` (name-status `-z`, R/C two-path aware) · `_staged_paths()` = content-bearing paths, deletions excluded · `_index_bytes()` via `git show :path` · `secret_guard(read=…)` with the worktree read retained as the default · commit gate moved to `_staged_entries` · `_git(errors="replace")`. **Declared scope call:** the MED index-vs-worktree finding rode along deliberately — it is the same line, and fixing the deletion without it would have left the guard structurally unsound. |
| 3 Suites | **586 backtest + 386 live** (was 586+376). |
| 4 Mutation | 6 mutants, all killed: deletion filter removed · `-z` dropped · index reader severed · commit gate reverted to `_staged_paths` · index oversize check disabled · index read failure swallowed. **M4 initially SURVIVED** — the deletion-only test wasn't isolated, because a first-ever sync also stages D13's `.gitignore` and D14's workflow files, so a commit fires whatever the gate reads. Test rewritten to settle those with one prior sync, then delete; M4 then killed. |
| 5 Line audit | Functional diff printed and accounted for line by line; no opportunistic edits beyond the declared MED above. |
| 6 Blast radius | Production callers of `sync_state`: `scripts/wheelbot_tick.sh:93` (intraday) and `:152` (EOD) — both unchanged signatures. `secret_guard`'s direct-call shape is unchanged and pinned by a new test; every existing caller is a test. `_git`'s `errors="replace"` is module-local. |
| 7 Plain language | A staged deletion is not a secret. The old code asked "can I open this file?" — a deleted file answers no, and "no" meant "credential", so the sync shot itself and kept shooting. It now asks git what is actually in the commit. |

**C2-equivalent receipt (the audit's own repro, re-run post-fix):** 3 consecutive `sync_state()`
calls, deleted tracked `.bak-2026-07-20`, real git — zero `treated as an offender`, zero
`sync ABORTED`; the only failure is the fake PAT failing GitHub auth against nonexistent repo
`o/r`, i.e. the network, not the guard. `still tracked? False` — the deletion committed. PAT
absent from all output (scrub held).

**Test-stub note (A3):** `test_sync_state_no_changes_still_pushes` gained a
`_staged_entries` stub alongside its `_staged_paths` one. The assertions are byte-identical;
only the stubbed seam moved, because the commit gate can no longer share the guard's
deletion-filtered list. Not a weakened test.
...
| Date | Entry |
|---|---|
| 2026-08-03 | **SESSION 5b — DEPLOYED AND RESET. The VPS now runs the repaired code; day 1 is one Schwab login away.** F5: 41/41 files byte-identical, VPS suite 384+1skip vs Mac 402 (delta = the 18 Mac-only dashboard tests, by design), units installed, timer deliberately left OFF. F7: `live/reset.py` written and executed — the July book (26 account dirs, gaps.jsonl, logs) archived to `data/archive-2026-08-03/`, config.json preserved, `.git` kept; fresh store carries only `logs/.epoch-2026-08-03`. **Two live defects caught by deploying rather than by auditing** — the git-identity gap (Mac-green/VPS-red, fixed `903ade0`) and the PAT workflow-scope trap (would have bricked the mirror on tick 1, resolved by seeding). F4a proved itself in production: 142 staged deletions committed and pushed clean. Owner ruled F1 rehearsal NOT run — day 1 is the rehearsal, watched. **Remaining before day 1: Schwab re-login ON THE VPS (token 7.99d, expired), then enable the timer.** |
| 2026-08-03 | **SESSION 5 — F4a DONE + code backed up.** Discovered the etf-bot repo had **no git remote at all**: 46 commits of repair work lived on one laptop with no copy. Pushed to private `GeorgeManavazian/wheelbot-code` after a credential scan of the tracked tree (all hits were regexes in `sync.py` or fake test fixtures). Discovered the **VPS is frozen at Jul-31 17:18** — every `live/*.py` differs by checksum and four modules (`chain_store`, `reconcile`, `run_chain_snapshot`, `escalations`) do not exist there; sessions 2-4 never left the Mac. VPS Schwab token is **7.99d = expired** (the 08-02 login was on the Mac, 0.59d). **F4a fixed** (`da803b6`): 10 red-first tests, 586+386 green, 6/6 mutants killed after M4 survived once and forced a proper deletion-only isolation; the audit's own repro re-run clean. Owner ruling this session: **stop auditing** — the 42 unverified F4 findings become a lookup table for when the live bot misbehaves, not a work queue. Next: F7 reset spec (owner), F1 rehearsal, F5 deploy. |
| 2026-08-02 | **A21 + A21b DONE (session 4).** Held-leg quotes moved to the RTH snapshot pass per the owner's same-session D1/D2/D3 rulings; 17:00 never touches the quote endpoint outside --smoke. 6 red-first tests → implementation → skeptic CORRECT-BUT-INCOMPLETE → 6 amendments same session (F1 MAJOR: one malformed position crashed the whole snapshot runner; F2 later-tick clobber; F3 corrupt-stats crash; F5 regex-only success pin; F6 stats-over-rows; F7 silent drift), F4 declared. 15/15 mutants killed — incl. MH2, a genuine survivor that proved lint pins cannot see dead code and forced the `resolve_held_merge` extraction + behavioral branch tests. Real-book receipts: 9 held legs derive from the 26 synced states; full pull→save→load→merge round-trip clean. C2 live pull impossible while paused (declared). Suites 586+376. |
| 2026-08-02 | **B6 DONE (session 4, commit `e4d68ac`).** Evidence sweep → owner approved ALL 11 retirements (PARA dead-since-2025-08 with vendor still serving frozen quotes = the silent-staleness class itself; WBA private; GPRO/PLUG/CHPT penny wrecks; 6 surgery-scarred). Universe 542→531, `_RETIRED` frozenset + leak test, 2 mutants killed. Declared: 210 names locally unvettable — live re-check on resume day. |
| 2026-08-02 | **OWNER DECISIONS — batch (ELI10 dialogs #4-5, session 4):** A20 scheduled DEAD LAST (post-repair, pre-day-1; 341-ticker fee-ON sweep = its evidence package) · B6 method = evidence sweep → kill-list → owner approves · A9b = keep as-is, declared (physically fillable, live never routes trend) · F9 = ACCEPT the resume-night alert flood ("let it bark") · C16b N=3 CONFIRMED · A10 D1-D4 CONFIRMED (freeze-and-ask over self-correct, after the NVDA-split story landed) · A8 D1/D2 CONFIRMED ($25 dust line). Remaining owner queue: Group A boundary review, Group C batch review, Q1-Q7 strategy questions (post-repair with A20). |
| 2026-08-02 | **OWNER DECISIONS — A21 (ELI10 dialog #3, session 4): D1 RTH / D2 carried-marks / D3 blessed.** Two-checker framing landed after two clarification rounds (intraday day-trader vs 5pm backstop). Owner's "it won't be dead anymore, we run on a VPS 24/7" recorded as an expectation, not a receipt — the ruling was taken on the "if it fires anyway, use real prices" argument. A21 + A21b now the next unblocked code work. |
| 2026-08-02 | **GROUP C DONE — C4+C5+C6+C7+C8+C10+C11 (session 4).** Tree found dirty a 4TH time with the whole batch in flight, no plan entry; standing rule held — 12/12 tests re-proven red on HEAD in a worktree (HEAD rendered Sharpe 99.48 from 3 obs; day-one dip = Max DD 0.00%; losing buy-back = "win"), 2 coverage holes plugged (C6 writer extracted + wiring pin, C7 clamp pin), 16/16 mutants killed. Skeptic BROKEN-narrowly → 3 amendments same session (F1 HIGH: DataFrame None→NaN killed the `is None` render guards, real store showed `nan% (0W/0L)`; F2: false "changed mid-series" on all-unstamped stores; F3: EW-basket copy) — each red-first + mutant-killed; could-not-break on the core incl. 25/25-account independent P&L recompute, 0 failures. F4-F9 declared. Suites 586+362. **Every non-owner-blocked code row in the plan is now DONE; what remains is owner decisions + Phase F.** |

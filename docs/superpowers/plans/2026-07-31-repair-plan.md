...
**Created:** 2026-07-31 · **Status:** IN PROGRESS — **59 of 76 done. EVERY non-owner-blocked code row is DONE.** (Session 4, 2026-08-02 evening: Group C finished — C4-C8, C10, C11 in one gated batch, see the Group C final batch block.) **Remaining work is decisions and Phase F only:** owner-blocked rows A20 · A21 (D1/D2/D3) + A21b · A9b · B6 (universe re-vet) · F9 (resume-day flood) · Group A boundary owner review · Group C batch owner review · A10 D1-D4 / C16b N=3 / A2-A3b provisional-default vetoes · A10d verifies on first live special dividend · resume-day step 0 = capture one raw quotes payload (C1b `quoteTime` name, needs fresh Schwab token) · then Phase F (F1-F9). Suites **586+362**. · **Bot state:** PAUSED
...
| A21 | `merge_held_legs` quote pull happens at 17:00, post-close (`run_daily.py:192` → `held_legs.py:113`) — same staleness class as A16 but feeds the EOD TP branch, so it is a TRADING input | MED | **TODO — UNBLOCKED. OWNER RULED 2026-08-02 (ELI10 dialog, session 4): D1 = marks AND EOD-backstop TP both on the RTH snapshot** (owner's frame accepted with the caveat recorded: he expects the backstop never to fire post-VPS; ruling taken on "if it fires, real prices" logic) · **D2 = held-pull-failed day steps with carried marks, backstop TP suspended that day, ONE alert** · **D3 = anchor break BLESSED** (state/snapshot/trade prices RTH-priced from the fix on; pre-break rows non-comparable, discarded at Phase F reset anyway). Implement per the analyst's option (b): held rows under a separate top-level `held_rows` key (never inside chains — F3 executed proof), `held_only=True` forced at load, A21b fixed inside this work. | Split out of A16 by owner decision 2026-07-31. Affects held-leg marks, snapshot equity (`snapshots.py:26`), and the EOD TP branch (`portfolio.py:94-101`); the intraday TP already runs on RTH quotes. |
...
| B6 | Re-vet the universe; add a `_RETIRED` frozenset with a test | HIGH | TODO — **owner ruled the METHOD 2026-08-02: evidence sweep (delisting/merger/symbol-reuse/liquidity per name) → kill-list → owner approves → `_RETIRED` + test.** Sweep agents are read-only; the frozenset lands through the one writer after approval. A10e's 26 fossils + verdicts.csv are prior art for the symbol-reuse check. | |
